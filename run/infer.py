...
def get_parser():
    ...
    parser.add_argument(
        "opts", default=None, help="see config/scannet/test_ours_openseg.yaml for all options", nargs=argparse.REMAINDER
    )
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    from dataset.feature_loader_inference import FusedFeatureInferenceLoader, collation_fn_eval_all

    val_data = FusedFeatureInferenceLoader(
...
def evaluate(model, val_data_loader, labelset_name="scannet_3d"):
    ...
    with torch.no_grad():
        ...
        for rep_i in range(args.test_repeats):
            ...
            output = []
            for i, (coords, feat, label, feat_3d, mask, inds_reverse, boxes, descriptions) in enumerate(
                ...
                labelset = [desc.text for desc in descriptions]
                text_features = extract_text_feature(labelset, args)
                ...
                if feature_type == "distill":
                    ...
                    pred = feat_ensemble.half() @ text_features.t()
                    pred = pred.detach().cpu().numpy()
                ...
                point_embeddings = predictions.detach().cpu()
                torch.save(
                    {"embeddings": point_embeddings, "boxes": [[box.to_dict(), indices] for box, indices in boxes]},
                    os.path.join(save_folder, f"{descriptions[0].scene_id}.pth"),
                )

                # collect scores per box
                box_scores = assign_clusters(boxes, pred)
                selected_boxes = np.argmax(
                    box_scores, axis=0
                )  # TODO: need to change this to support zero or multiple boxes
            ...
            with open(os.path.join(save_folder, args.output_name.format(rep_i)), "w") as f:
