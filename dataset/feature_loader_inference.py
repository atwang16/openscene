...
class FusedFeatureInferenceLoader(Point3DLoader):
    """Dataloader for fused point features."""
    ...
    def __init__(
        ...
        *,
        ...
        self.scenes = SceneCollection("ovfgvg", metadata_path, splits=[self.metadata_split])
        ...
        if "nuscenes" in self.dataset_name:  # only one file for each scene
            ...
            for data_path in self.data_paths:
                if "scannet" in self.dataset_name:
                    ...
                    scene_name = data_path[:-4].split("/")[-1]
                if (
                    scene_name in self.scenes.scene_ids[self.metadata_split]
                ):  # filter out scenes for which we don't have prompts
                    file_dirs = glob(join(self.datapath_feat, scene_name + "_*.pt"))
                    self.list_occur.append(len(file_dirs))  # 1 if file was found in datapath_feat, 0 otherwise
                else:
                    self.list_occur.append(0)  # exclude file
    ...
    def __getitem__(self, index_long):
        ...
        if self.use_shm:
            locs_in = SA.attach(
                "shm://%s_%s_%06d_locs_%08d" % (self.dataset_name, self.split, self.identifier, index)
            ).copy()
            feats_in = SA.attach(
                "shm://%s_%s_%06d_feats_%08d" % (self.dataset_name, self.split, self.identifier, index)
            ).copy()
            labels_in = SA.attach(
                "shm://%s_%s_%06d_labels_%08d" % (self.dataset_name, self.split, self.identifier, index)
            ).copy()
        else:
        ...
        if "nuscenes" not in self.dataset_name:
            ...
            processed_data = torch.load(join(self.datapath_feat, scene_name + "_%d.pt" % (nn_occur)))
        else:
            # no repeated file
        ...
        # load text features
        scene = self.scenes.get_scene_by_id(scene_name)
        descriptions = self.scenes.get_annotations_by_scene(self.metadata_split, scene_name)
        boxes = []
        for box in scene.boxes:
            vertex_mask = box.contains(locs_raw)
        ...
        return coords, feats, labels, feat_3d, mask, boxes, descriptions
...
def collation_fn(batch):
    ...
    """
    coords, feats, labels, feat_3d, mask_chunk, boxes, descriptions = list(zip(*batch))
...
def collation_fn_eval_all(batch):
    ...
    """
    coords, feats, labels, feat_3d, mask, inds_recons, boxes, descriptions = list(zip(*batch))
